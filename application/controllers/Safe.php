...
class Safe extends WebLoginBase_Controller {
	...
	public final function setPasswddo(){
		...
		$sql="update {$this->prename}members set password='".md5($npwd)."' where uid={$this->user['uid']}";
    ...
    public final function setCoinPwddo(){
        ...
        $sql="select password, coinPassword from {$this->prename}members where uid={$this->user['uid']}";
        ...
        $sql="update {$this->prename}members set coinPassword='{$npwd}' where uid={$this->user['uid']}";
