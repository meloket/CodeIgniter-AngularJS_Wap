...
class Data extends WebLoginBase_Controller {

	public function __construct()
    {
        parent::__construct();
        $this->load->library('bet');
    }
    public final function postcodeagent()
    {
    	$money = $_POST['money'];
    	$betid = $_POST['betid'];
    	$userid = $_POST['userid'];
		...
		$betdata=$this->getRow("select * from {$this->prename}bets where id={$betid} limit 0,1", $betid);

		$actionTime=$this->getGameActionTime($betdata['type']);
		$liqType=101;
		$info='投注';

    	$newuser = $this->getRow("select * from {$this->prename}members where uid={$userid} limit 0,1", $userid);
    	
    	try{
			$randorderid=mt_rand(10000,99999);
			
			$wjorderId = date('YmdHis',$this->time).strval($this->createRandomStr(10));
			$orderId = strval(date('YmdHis',$this->time)).$randorderid;
			...
			'type'=> $betdata['type'],
			'playedGroup'=> $betdata['playedGroup'],
			'playedId'=> $betdata['playedId'],
			'Groupname'=> $betdata['Groupname'],
			//'playedname'=>$playeds['name'],
			'odds'=> $betdata['odds'],
			'actionNo'=> $betdata['actionNo'],
			'actionTime'=>$this->time,
			'actionIP'=>$this->ip(true),
			'actionData' => $betdata['actionData'],
			'rebate' => $betdata['rebate'],
			'money' => $money,
			'kjTime'=>$actionTime
			);

			$lastedInsertid = $this->insertRow($this->prename .'bets', $para);
	...
	public final function postcodea(){
		...
		try{
			...
			foreach($betBean as $code){
				...
				$sql=$code['wjorderId'].', ';
				$playeds = $this->playeds[$code['playId']];
				$playedGroup = $this->playedGroup[(isset($playeds['played_groupid']) ? $playeds['played_groupid'] : NULL)];
				...
				//判断是否复选
				if((isset($code['betInfo']) ? $code['betInfo'] : NULL)){
